...
export function HorariosDocentes() {
  ...
  useEffect(() => {
    const cargarDocentes = async () => {
      console.log("lapso", lapsoActual?.nombre_lapso);
      
      try {
        const response = await Api.get(`/docentes/con_clases/${lapsoActual?.nombre_lapso}`);
  ...
  const descargarHorario = async (docenteId, trimestreId = null, nombre_completo, nombre_trimestre, lapso = null) => {
    
    // Funcion para reemplazar espacios por "_" en nombre_completo
    nombre_completo = nombre_completo.replace(/\s/g, "_");

    try {
      const url_descargar = `/generar_pdf_docente/${docenteId}/${trimestreId}/${lapso}`;
    ...
    },
    {
      ...
      cell: (row) => (
        <Buttom
          ...
          onClick={() => descargarHorario(row.docente_id, row.trimestre_id, row.nombre_completo, row.trimestre_nombre, lapsoActual?.nombre_lapso)}
          title="Descargar"
          text="Descargar PDF"
        />
      ),
